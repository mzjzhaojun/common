...
public class PoiExcelExport {
	...
	public PoiExcelExport(HttpServletResponse response, String fileName) {
		this.fileName = fileName + "导出-" + DateTimeUtil.getDateTime();
	 ...
	 *            数据
	 */
	public void wirteExcel(String export_code, String export_value, List<Map<String, Object>> dataList) {
		String titleColumn[] = export_code.split(",");
		String titleName[] = export_value.split(",");
		...
		try {
			...
			for (int i = 0; i < titleName.length; i++) {
				sheet.setColumnWidth(i, 13 * 256); // 设置宽度
					...
					for (int rowIndex = 1; rowIndex <= dataList.size(); rowIndex++) {
						...
						for (int columnIndex = 0; columnIndex < titleColumn.length; columnIndex++) {
							...
							if (!"".equals(title)) { // 字段不为空
								...
								// 设置要执行的方法
								// Method method =
								// clsss.getDeclaredMethod(methodName);
								// 获取返回类型
								// String returnType =
								// method.getReturnType().getName();
								Object data = map.get(title) == null ? "" : map.get(title);
								...
								if (data != null && !"".equals(data)) {
									...
									if (data instanceof Integer) {
										...
										cell.setCellValue(doubleDecimalFormat.format(d));
									} else if (data instanceof Float) {
										float f = ((Float) data).floatValue();
										cell.setCellValue(floatDecimalFormat.format(f));
